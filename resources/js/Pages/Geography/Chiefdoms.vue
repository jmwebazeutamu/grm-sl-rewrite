...
const props = defineProps({
    ...
    chiefdoms: { type: Array, default: () => [] },
});

const level = 'chiefdoms';
const editingId = ref(null);
const editForm = useForm({ name: '' });
const showAdd = ref(false);
const addForm = useForm({ name: '', parent_id: props.district.id });
...
<template>
    <Head :title="`${district.name} — Chiefdoms`" />
    <AppLayout>
        <div class="max-w-4xl">
            <nav class="text-sm text-slate-500 mb-3">
                ...
                <span class="mx-1">›</span>
                <span class="text-slate-700">{{ district.name }}</span>
            </nav>
            <div class="flex items-start justify-between mb-6">
                <h1 class="text-2xl font-semibold text-slate-900">Chiefdoms in {{ district.name }}</h1>
                <button type="button" class="px-3 py-2 rounded bg-slate-900 text-white text-sm hover:bg-slate-800" @click="showAdd = !showAdd">
                    {{ showAdd ? 'Cancel' : 'Add chiefdom' }}
                </button>
            </div>
            ...
            <table class="w-full bg-white border border-slate-200 rounded-lg">
                <thead class="text-left text-sm text-slate-600 border-b border-slate-200">
                    <tr>
                        ...
                        <th class="px-4 py-3 w-32">Sections</th>
                        <th class="px-4 py-3 w-40 text-right"></th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="c in chiefdoms" :key="c.id" class="border-b border-slate-100 last:border-0">
                        <td class="px-4 py-3">
                            <template v-if="editingId === c.id">
                                <input v-model="editForm.name" type="text" class="w-full rounded border-slate-300 text-sm" />
                            </template>
                            <template v-else>
                                <Link :href="route('admin.geography.chiefdoms.show', c.id)" class="text-slate-900 hover:underline">{{ c.name }}</Link>
                            </template>
                        </td>
                        <td class="px-4 py-3 text-sm text-slate-600">{{ c.children_count }}</td>
                        <td class="px-4 py-3 text-right space-x-2">
                            <template v-if="editingId === c.id">
                                <button type="button" class="px-2.5 py-1 rounded bg-slate-900 text-white text-xs" @click="saveEdit(c)">Save</button>
                                <button type="button" class="text-xs text-slate-600 underline" @click="cancelEdit">Cancel</button>
                            </template>
                            <template v-else>
                                <button type="button" class="text-xs text-slate-600 hover:text-slate-900 underline" @click="startEdit(c)">Edit</button>
                                <button type="button" class="text-xs text-rose-600 hover:text-rose-800 underline" @click="destroy(c)">Delete</button>
                            </template>
                        </td>
                    </tr>
                    <tr v-if="showAdd" class="bg-slate-50">
                        <td class="px-4 py-3"><input v-model="addForm.name" type="text" placeholder="Chiefdom name" class="w-full rounded border-slate-300 text-sm" /></td>
                        <td></td>
                        <td class="px-4 py-3 text-right">
                            <button type="button" class="px-2.5 py-1 rounded bg-slate-900 text-white text-xs" @click="submitAdd">Save</button>
                        </td>
                    </tr>
                    <tr v-if="chiefdoms.length === 0 && !showAdd">
                        <td colspan="3" class="px-4 py-6 text-center text-slate-500 text-sm">No chiefdoms.</td>
                    </tr>
                </tbody>
            </table>
        </div>

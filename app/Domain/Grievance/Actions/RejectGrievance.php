...
/**
 * Rejects a grievance. If still in `submitted`, auto-chains
 * `submitted → under_review → rejected` so the reviewer doesn't
 * need a separate "pick up" click.
 */
class RejectGrievance
{
    public function __construct(private readonly GrievanceWorkflow $workflow)
    {
    }

    public function __invoke(Grievance $grievance, User $actor, string $reason): Grievance
    {
        return DB::transaction(function () use ($grievance, $actor, $reason): Grievance {
